 ...
 *   - Scrollable block palette without page splitting
 ...
 *   - Variables, condition variables, conditional values and upgrade sorting
 *   - Sort target inspection blocks for individual buildings and selected upgrades
 */
(function () {
    'use strict';
    ...
    var RESET_MARKER_KEY = 'CookieProgrammerMod.v01.reset_0_1_22';
    var ROOT_ID = 'cp-root';
    ...
    var SYNERGY_TARGETS = [
        {id:'upgrade:futureAlmanacs',label:'農場シナジー1（5%農場、0.1%タイムマシン）',name:'Future almanacs'},
        {id:'upgrade:rainPrayer',label:'農場シナジー2（5%農場、0.1%神殿）',name:'Rain prayer'},
        {id:'upgrade:seismicMagic',label:'鉱山シナジー1（5%鉱山、0.1%魔法使いの塔）',name:'Seismic magic'},
        {id:'upgrade:asteroidMining',label:'鉱山シナジー2（5%鉱山、0.1%出荷）',name:'Asteroid mining'},
        {id:'upgrade:quantumElectronics',label:'工場シナジー1（5%工場、0.1%反物質凝縮器）',name:'Quantum electronics'},
        {id:'upgrade:temporalOverclocking',label:'工場シナジー2（5%工場、0.1%タイムマシン）',name:'Temporal overclocking'},
        {id:'upgrade:contractsFromBeyond',label:'銀行シナジー1（5%銀行、0.1%ポータル）',name:'Contracts from beyond'},
        {id:'upgrade:printingPresses',label:'工場シナジー3（5%工場、0.1%銀行）',name:'Printing presses'},
        {id:'upgrade:paganism',label:'神殿シナジー1（5%神殿、0.1%ポータル）',name:'Paganism'},
        {id:'upgrade:godParticle',label:'神殿シナジー2（5%神殿、0.1%反物質凝縮器）',name:'God particle'},
        {id:'upgrade:arcaneKnowledge',label:'魔法使いの塔シナジー1（5%魔法使いの塔、0.1%錬金術室）',name:'Arcane knowledge'},
        {id:'upgrade:magicalBotany',label:'農場シナジー3（5%農場、0.1%魔法使いの塔）',name:'Magical botany'},
        {id:'upgrade:fossilFuels',label:'鉱山シナジー3（5%鉱山、0.1%出荷）',name:'Fossil fuels'},
        {id:'upgrade:shipyards',label:'工場シナジー4（5%工場、0.1%出荷）',name:'Shipyards'},
        {id:'upgrade:primordialOres',label:'鉱山シナジー4（5%鉱山、0.1%錬金術室）',name:'Primordial ores'},
        {id:'upgrade:goldFund',label:'銀行シナジー2（5%銀行、0.1%錬金術室）',name:'Gold fund'},
        {id:'upgrade:infernalCrops',label:'農場シナジー4（5%農場、0.1%ポータル）',name:'Infernal crops'},
        {id:'upgrade:abysmalGlimmer',label:'ポータルシナジー1（5%ポータル、0.1%プリズム）',name:'Abysmal glimmer'},
        {id:'upgrade:relativisticParsecSkipping',label:'出荷シナジー1（5%出荷、0.1%タイムマシン）',name:'Relativistic parsec-skipping'},
        {id:'upgrade:primevalGlow',label:'タイムマシンシナジー1（5%タイムマシン、0.1%プリズム）',name:'Primeval glow'},
        {id:'upgrade:extraPhysicsFunding',label:'銀行シナジー3（5%銀行、0.1%反物質凝縮器）',name:'Extra physics funding'},
        {id:'upgrade:chemicalProficiency',label:'錬金術室シナジー1（5%錬金術室、0.1%反物質凝縮器）',name:'Chemical proficiency'},
        {id:'upgrade:lightMagic',label:'魔法使いの塔シナジー2（5%魔法使いの塔、0.1%プリズム）',name:'Light magic'},
        {id:'upgrade:mysticalEnergies',label:'神殿シナジー3（5%神殿、0.1%プリズム）',name:'Mystical energies'},
        {id:'upgrade:perforatedMilleFeuilleCosmos',label:'ポータルシナジー2（5%ポータル、0.1%アイドルバース）',name:'Perforated mille-feuille cosmos'},
        {id:'upgrade:infraversesAndSuperverses',label:'フラクタルエンジンシナジー1（5%フラクタルエンジン、0.1%アイドルバース）',name:'Infraverses and superverses'},
        {id:'upgrade:thoughtsAndPrayers',label:'神殿シナジー4（5%神殿、0.1%コーテックス・ベーカー）',name:'Thoughts & prayers'},
        {id:'upgrade:fertileMinds',label:'農場シナジー5（5%農場、0.1%コーテックス・ベーカー）',name:'Fertile minds'},
        {id:'upgrade:acceleratedDevelopment',label:'タイムマシンシナジー2（5%タイムマシン、0.1%You）',name:'Accelerated development'},
        {id:'upgrade:peerReview',label:'Javascriptコンソールシナジー1（5%Javascriptコンソール、0.1%You）',name:'Peer review'}
    ];
    function getSynergyTarget(id){
        for (var i=0;i<SYNERGY_TARGETS.length;i++) if (SYNERGY_TARGETS[i].id===id) return SYNERGY_TARGETS[i];
        return null;
    }
    var BLOCK_TYPES = {
        ...
        sortDefinition: { label: 'ソートを定義する', kind: 'sortSetting', color: '#70a6d8', category: 'control' },
        variableDefinition: { label: '数値変数を定義する', kind: 'variableDefinition', color: '#b07bd8', category: 'control' },
        ...
        sortBuildingBaseCps: { label: '施設の基礎CPS', kind: 'value', color: '#e8c45b', category: 'info' },
        sortBuildingCount: { label: '施設数', kind: 'value', color: '#e8c45b', category: 'info' },
        sortBuildingPrice: { label: '施設の値段', kind: 'value', color: '#e8c45b', category: 'info' },
        sortUpgradePrice: { label: 'アップグレードの値段', kind: 'value', color: '#e8c45b', category: 'info' },
        sortBuildingUpgradeMultiplier: { label: '施設アップグレード総倍率', kind: 'value', color: '#e8c45b', category: 'info' },
        sortTargetAcquired: { label: 'ソート対象の取得状況', kind: 'detection', color: '#c58bd8', category: 'detection' },
        ...
        branch: { label: 'もし〜なら', kind: 'value', color: '#b07bd8', category: 'variables' },
        sortTarget: { label: 'ソート対象を調べる', kind: 'value', color: '#c58bd8', category: 'detection' },
    ...
    function defaultBlock(type, x, y) {
        ...
        if (type === 'branch') {
            block.condition = null; block.trueValue = '0'; block.falseValue = '0';
            block.inputs = { condition: null, trueValue: null, falseValue: null };
        }
        if (type === 'sortTarget') block.target = 'building:0';
        if (type === 'variableDefinition') {
            block.variableId = state.variables[0] ? state.variables[0].id : '';
            block.inputs = { value: null };
        }
        if (type === 'sortDefinition') {
            block.sortId = state.sorts[0] ? state.sorts[0].id : '';
            block.targetType = 'upgrades'; block.order = 'desc';
            block.inputs = { value: null };
        }
        if (type === 'and' || type === 'or') {
            block.inputs = { a: null, b: null };
    ...
    function sanitizeState() {
        ...
        delete state.conditionVariables;
        refreshVariableBlockTypes();
        state.variables.forEach(function(v){ ensureVariableDefinition(v); });
        ...
        state.programs.forEach(function (p) {
            ...
            p.blocks.forEach(function (b) {
                ...
                expressionInputKeys(b).forEach(function(key){
                ...
                if (b.type === 'if') {
                    ...
                    delete b.inputs.left; delete b.inputs.right;
                    delete b.left; delete b.right; delete b.op;
                }
            });
        });

        if (!state.programs.length) state.currentId = null;
        else if (!state.currentId || !state.programs.some(function (p) { return p.id === state.currentId; })) state.currentId = state.programs[0].id;
    }
    ...
    function getEditorDocument() {
        ...
        if (runtime.editorMode === 'variable') return getVariable(runtime.selectedVariableId);
    ...
    function injectStyle() {
        ...
        style.id = STYLE_ID;
        style.textContent = `
#${ROOT_ID}{position:relative;width:min(100%,100vh);height:min(100%,100vh);aspect-ratio:1 / 1;min-height:260px;z-index:1;display:flex;font-family:Arial,"Trebuchet MS",sans-serif;color:#333;touch-action:none;box-sizing:border-box}
#${ROOT_ID} .cp-dim{display:none}
#${ROOT_ID} .cp-window{position:relative;width:100%;height:100%;aspect-ratio:1 / 1;min-height:260px;background:#eee;border:0;border-radius:0;box-shadow:none;overflow:hidden;display:flex;flex-direction:column}
...
#${ROOT_ID} .cp-sidebar{width:270px;background:#ddd;border-right:3px solid #aaa;padding:10px;box-sizing:border-box;overflow:hidden;display:flex;flex-direction:column;min-height:0}
...
#${ROOT_ID} .cp-palette-tabs,#${ROOT_ID} .cp-palette-pages{display:none}
...
#${ROOT_ID} .cp-palette-list-wrap{flex:1;min-height:80px;overflow-y:auto;overflow-x:hidden;background:rgba(0,0,0,.55);padding:4px 6px;border-radius:5px;overscroll-behavior-y:auto;-webkit-overflow-scrolling:touch}
...
#${ROOT_ID} .cp-canvas-wrap{position:relative;flex:1;min-width:0;min-height:0;background:#f7f1df;overflow:hidden;touch-action:none;cursor:grab}
...
#${ROOT_ID}{width:min(100%,100vh);height:min(100%,100vh)}
#${ROOT_ID} .cp-window{width:100%;height:100%;border-radius:0;border-width:0}
...
`;
    ...
    function ensureRoot(parent) {
        ...
        if (root) {
            ...
            var oldPalettePages = root.querySelector('.cp-palette-pages');
            if (oldPalettePages) oldPalettePages.remove();
            var oldPaletteList = root.querySelector('.cp-palette-list');
            if (oldPaletteList && !oldPaletteList.parentElement.classList.contains('cp-palette-list-wrap')) {
                var paletteWrap = document.createElement('div');
                paletteWrap.className = 'cp-palette-list-wrap';
                oldPaletteList.parentNode.insertBefore(paletteWrap, oldPaletteList);
                paletteWrap.appendChild(oldPaletteList);
            }
...
<div class="cp-window">
  ...
  <div class="cp-body">
    <aside class="cp-sidebar">
      ...
      <div class="cp-variable-tools" style="margin-top:6px"></div>
      ...
      <div class="cp-palette-list-wrap">
        <div class="cp-palette-list"></div>
      </div>
      <div class="cp-help-source">配置：ブロックをタップ/クリックすると追加。追加後はドラッグで移動できます。<br><br>接続：先に実行するブロックをクリック → 後に実行するブロックをクリック。複数接続できます。<br><br>もし〜なら：条件に合えば1本目、合わなければ2本目の接続へ進みます。<br><br>演算：数値欄には直接入力でき、演算・情報・変数・検知ブロックも入れ子にできます。<br><br>検知：かつ・または・ではないを使って条件を組み合わせられます。<br><br>削除：ブロックを選択してブロック削除。PCではDeleteキーでも削除できます。<br><br>ソート：ソートを作成し、「ソートを定義する」ブロックに数値変数や計算式を入れて評価値を作ります。条件変数と「もし〜なら」を組み合わせて計算式を分岐できます。</div>
    </aside>
  ...
  <div class="cp-modal">
    ...
    function expressionInputKeys(b) {
        ...
        if (b.type === 'variableDefinition' || b.type === 'sortDefinition') return ['value'];
        if (b.type === 'branch') return ['condition','trueValue','falseValue'];
        ...
        if (['sortBuildingBaseCps','sortBuildingCount','sortBuildingPrice','sortUpgradePrice','sortBuildingUpgradeMultiplier','sortTargetAcquired'].indexOf(b.type)>=0) return ['targetRef'];
        return [];
    }
    ...
    function expressionLabel(p, id, seen) {
        ...
        var def = BLOCK_TYPES[b.type];
        if (!def) return '';
        if (b.type === 'sortTargetAcquired') {
            var acquiredRef = getInputRef(b,'targetRef');
            var acquiredLabel = acquiredRef ? expressionLabel(p, acquiredRef, seen) : '調べる対象';
            return '<span class="cp-nested-expression cp-expression-child" data-expression-id="' + escapeHtml(b.id) + '" style="background:' + escapeHtml(def.color) + '">取得済み？ ' + acquiredLabel + '</span>';
        }
        if (b.type === 'sortTarget') {
            var targetLabels = { 'building:0':'カーソル','building:1':'グランマ','building:2':'農場','building:3':'鉱山','building:4':'工場','building:5':'銀行','building:6':'神殿','building:7':'魔法使いの塔','building:8':'出荷','building:9':'錬金術室','building:10':'ポータル','building:11':'タイムマシン','building:12':'反物質凝縮器','building:13':'プリズム','building:14':'チャンスメーカー','building:15':'フラクタルエンジン','building:16':'Javascriptコンソール','building:17':'アイドルバース','building:18':'コーテックス・ベーカー','building:19':'あなた','special:chocolateEgg':'チョコレートエッグ','upgrade:oneMind':'One mind','upgrade:communalBrainsweep':'Communal brainsweep','upgrade:elderPact':'Elder Pact' };
            var synergyTarget = getSynergyTarget(b.target);
            var shownTargetLabel = synergyTarget ? synergyTarget.label : (targetLabels[b.target] || '対象');
            return '<span class="cp-nested-expression cp-expression-child" data-expression-id="' + escapeHtml(b.id) + '" style="background:' + escapeHtml(def.color) + '">ソート対象：' + escapeHtml(shownTargetLabel) + '</span>';
        }
        if (b.type === 'branch') {
            var branchCondition = inputSlotHtml(p,b,'condition',seen);
            var branchTrue = inputSlotHtml(p,b,'trueValue',seen);
            var branchFalse = inputSlotHtml(p,b,'falseValue',seen);
            return '<span class="cp-nested-expression cp-expression-child" data-expression-id="' + escapeHtml(b.id) + '" style="background:' + escapeHtml(def.color) + '"><span class="cp-expression-inline">もし ' + branchCondition + ' なら ' + branchTrue + ' ／ それ以外 ' + branchFalse + '</span></span>';
        }
    ...
    function inputSlotHtml(p, b, key, seen) {
        ...
        if (key === 'targetRef') return '<span>調べる対象を入れる</span>';
        var value = key === 'a' ? b.a : (key === 'b' ? b.b : (key === 'value' ? b.value : (key === 'trueValue' ? b.trueValue : (key === 'falseValue' ? b.falseValue : '0'))));
    ...
    function blockInnerHtml(b) {
        ...
        if (b.type === 'branch') {
            return head + '<div style="text-align:center"><b>もし〜なら</b></div>'
                + '<div>もし ' + inputSlotHtml(p,b,'condition') + ' なら</div>'
                + '<div>→ ' + inputSlotHtml(p,b,'trueValue') + '</div>'
                + '<div>それ以外 → ' + inputSlotHtml(p,b,'falseValue') + '</div>';
        }
        if (b.type === 'variableDefinition') {
            var vv = getVariable(b.variableId);
            return head + '<div style="text-align:center"><b>数値変数「' + escapeHtml(vv ? vv.name : '変数') + '」を定義する</b></div><div class="cp-expression-row">＝ ' + inputSlotHtml(p,b,'value') + '</div>';
        }
        if (b.type === 'sortDefinition') {
            var sortObj = getSort(b.sortId) || (runtime.editorMode === 'sort' ? getSort(runtime.selectedSortId) : null);
            var st = sortObj ? sortObj.targetType : (b.targetType || 'upgrades');
            var so = sortObj ? sortObj.order : (b.order || 'desc');
            return head + '<div style="text-align:center"><b>ソートを定義する</b></div>'
                + '<div>対象：<select class="cp-select" data-field="targetType"><option value="upgrades"'+(st==='upgrades'?' selected':'')+'>アップグレード</option><option value="buildings"'+(st==='buildings'?' selected':'')+'>施設</option></select></div>'
                + '<div>ソート用の計算結果：' + inputSlotHtml(p,b,'value') + '</div>'
                + '<div>並び順：<select class="cp-select" data-field="order"><option value="desc"'+(so==='desc'?' selected':'')+'>大きい順</option><option value="asc"'+(so==='asc'?' selected':'')+'>小さい順</option></select></div>';
        }
        if (b.type === 'sortTarget') {
            var targetOptions = [
                {id:'building:0',label:'カーソル'}, {id:'building:1',label:'グランマ'}, {id:'building:2',label:'農場'}, {id:'building:3',label:'鉱山'}, {id:'building:4',label:'工場'}, {id:'building:5',label:'銀行'}, {id:'building:6',label:'神殿'}, {id:'building:7',label:'魔法使いの塔'}, {id:'building:8',label:'出荷'}, {id:'building:9',label:'錬金術室'}, {id:'building:10',label:'ポータル'}, {id:'building:11',label:'タイムマシン'}, {id:'building:12',label:'反物質凝縮器'}, {id:'building:13',label:'プリズム'}, {id:'building:14',label:'チャンスメーカー'}, {id:'building:15',label:'フラクタルエンジン'}, {id:'building:16',label:'Javascriptコンソール'}, {id:'building:17',label:'アイドルバース'}, {id:'building:18',label:'コーテックス・ベーカー'}, {id:'building:19',label:'あなた'},
                {id:'special:chocolateEgg',label:'チョコレートエッグ'}, {id:'upgrade:oneMind',label:'One mind'}, {id:'upgrade:communalBrainsweep',label:'Communal brainsweep'}, {id:'upgrade:elderPact',label:'Elder Pact'}
            ];
            SYNERGY_TARGETS.forEach(function(t){ targetOptions.push({id:t.id,label:t.label}); });
            return head + '<div style="text-align:center"><b>ソート対象を調べる</b></div><select class="cp-select" data-field="target">' + optionsHtml(targetOptions,b.target) + '</select>';
        }
        if (b.type === 'sortTargetAcquired') {
            return head + '<div style="text-align:center"><b>ソート対象の取得状況</b></div><div class="cp-expression-row">' + inputSlotHtml(p,b,'targetRef') + '</div>';
        }
        if (b.type === 'repeat') {
        ...
        if (['sortBuildingBaseCps','sortBuildingCount','sortBuildingPrice','sortUpgradePrice','sortBuildingUpgradeMultiplier','sortTargetAcquired'].indexOf(b.type)>=0) {
            return head + '<div class="cp-nested-expression" style="background:' + escapeHtml(def.color) + '">' + escapeHtml(def.label) + '</div><div class="cp-expression-row">対象：' + inputSlotHtml(p,b,'targetRef') + '</div>';
        }
    ...
    function bindBlockFields(el, block) {
        el.querySelectorAll('[data-field]').forEach(function (field) {
            ...
            field.addEventListener('change', function () {
                block[this.dataset.field] = this.value;
                if (block.type === 'sortDefinition' && runtime.editorMode === 'sort') {
                    ...
                    if (sort) {
                        ...
                        if (block.type === 'sortDefinition') { sort.definitionBlockId = block.id; }
                    }
                }
                saveState();
                renderCanvas();
    ...
    function getPaletteTypes() {
        return Object.keys(BLOCK_TYPES).filter(function (type) {
            ...
            if (runtime.editorMode === 'sort') {
                if (runtime.paletteCategory === 'control') return type === 'sortDefinition';
            ...
            if (runtime.editorMode === 'variable') {
                if (runtime.paletteCategory === 'control') return false;
                if (['operator','info','variables','detection'].indexOf(runtime.paletteCategory) < 0) return false;
                return def.category === runtime.paletteCategory && ['value','operator','comparison','logic','detection'].indexOf(def.kind) >= 0;
            }
    ...
    function renderPalette() {
        ...
        if (!list.children.length) {
            ...
            empty.textContent = 'このカテゴリにはまだブロックがありません。';
    ...
    function getViewBounds() {
        ...
        var margin = 40;
        return {
            minX: scaledW + margin * 2 <= w ? (w - scaledW) / 2 : w - scaledW - margin,
            maxX: scaledW + margin * 2 <= w ? (w - scaledW) / 2 : margin,
            minY: scaledH + margin * 2 <= h ? (h - scaledH) / 2 : h - scaledH - margin,
            maxY: scaledH + margin * 2 <= h ? (h - scaledH) / 2 : margin
        };
    }

    function handoffToCookieClicker(deltaY) {
        if (!deltaY) return;
    ...
    function clampView() {
        ...
        if (!bounds) return;
        runtime.view.x = Math.min(bounds.maxX, Math.max(bounds.minX, runtime.view.x));
        runtime.view.y = Math.min(bounds.maxY, Math.max(bounds.minY, runtime.view.y));
    }
    ...
    function addBlock(type, x, y) {
        ...
        var def = BLOCK_TYPES[type];
        if (runtime.editorMode === 'sort') {
            if (type !== 'sortDefinition' && !def) return;
            if (type !== 'sortDefinition' && ['value','operator','comparison','logic','detection'].indexOf(def.kind) < 0) { setStatus('ソートには値・演算・条件ブロックを使ってください'); return; }
            if (type === 'sortDefinition' && p.blocks.some(function(x){ return x.type === 'sortDefinition'; })) { setStatus('ソート定義ブロックは1つだけです'); return; }
        }
        if (runtime.editorMode === 'variable' && type !== 'variableDefinition') {
            if (!def || ['value','operator','comparison','logic','detection'].indexOf(def.kind) < 0) return;
        }
        var b = defaultBlock(type, Math.max(5, Math.min(WORLD_SIZE - 240, x - 90)), Math.max(5, Math.min(WORLD_SIZE - 80, y - 20)));
        if (type === 'sortTargetAcquired') b.inputs = { targetRef: null };
        if (runtime.editorMode === 'sort' && type === 'sortDefinition') {
            b.sortId = p.id; b.targetType = p.targetType || 'upgrades'; b.order = p.order || 'desc'; b.inputs = { value: null }; p.definitionBlockId = b.id;
        }
        if (type === 'click') b.target = 'bigCookie';
        if (type === 'buy') b.target = 'building:0';
        if (type === 'variableAdd' || type === 'variableSet') { b.variableId = state.variables[0] ? state.variables[0].id : ''; b.value = '0'; b.inputs = { value: null }; }
        if (type === 'playTime') b.unit = 'seconds';
        if (type === 'inventory') b.target = 'cookies';
        if (type === 'buyUpgrade') { b.sortId = state.sorts[0] ? state.sorts[0].id : ''; b.unavailableMode = 'next'; }
        p.blocks.push(b);
        if (runtime.expressionTarget && isExpressionType(type)) {
            var targetParent = getBlock(p, runtime.expressionTarget.parentId);
            var allowed = true;
            if (targetParent && targetParent.type === 'if' && runtime.expressionTarget.key === 'condition') allowed = isConditionType(type);
            if (targetParent && targetParent.type === 'branch' && runtime.expressionTarget.key === 'condition') allowed = isConditionType(type);
            if (allowed && setExpressionInput(p, runtime.expressionTarget.parentId, runtime.expressionTarget.key, b.id)) { runtime.expressionTarget = null; saveState(); renderCanvas(); setStatus('ブロックを入れました'); return b; }
            p.blocks = p.blocks.filter(function (x) { return x.id !== b.id; }); runtime.expressionTarget = null; setStatus('このブロックはその入力には入れられません'); return null;
        }
        saveState(); renderCanvas(); return b;
    }
    ...
    function ensureVariableDefinition(v) {
        if (!v) return null;
        if (!Array.isArray(v.blocks)) v.blocks = [];
        if (!Array.isArray(v.connections)) v.connections = [];
        var root = v.blocks.find(function(b){ return b.type === 'variableDefinition'; });
        if (!root) { root = defaultBlock('variableDefinition',80,80); root.variableId=v.id; v.blocks.push(root); }
        v.definitionRootId = root.id;
        return root;
    }

    function editVariableDefinition() {
        var list=state.variables||[]; if(!list.length){setStatus('数値変数がありません');return;}
        var v=list[state.variables.findIndex(function(x){return x.id===runtime.selectedVariableId;})] || list[list.length-1];
        runtime.selectedVariableId=v.id; ensureVariableDefinition(v); runtime.editorMode='variable'; runtime.selectedBlockId=null; runtime.connectionFrom=null; runtime.expressionTarget=null; renderVariableTools(); renderEditorModeUI(); renderPalette(); renderCanvas(); setStatus('数値変数「'+v.name+'」の定義を編集しています');
    }

    function renderVariableTools() {
        var root=document.getElementById(ROOT_ID); if(!root)return;
        var box=root.querySelector('.cp-variable-tools'); if(!box)return;
        var nv=(state.variables||[]).map(function(v){return {id:v.id,label:v.name};});
        if(!runtime.selectedVariableId && nv.length) runtime.selectedVariableId=nv[0].id;
        box.innerHTML='<div style="width:100%"><b>数値変数</b><div style="display:flex;gap:3px;margin-top:3px"><select class="cp-select cp-variable-select" style="flex:1">'+optionsHtml(nv,runtime.selectedVariableId|| (nv[0]&&nv[0].id))+'</select><button class="cp-btn cp-add-variable">追加</button><button class="cp-btn cp-rename-variable">名前変更</button><button class="cp-btn cp-edit-variable-definition">定義</button></div></div>';
        var vs=box.querySelector('.cp-variable-select'); if(vs)vs.addEventListener('change',function(){runtime.selectedVariableId=this.value;});
        box.querySelector('.cp-add-variable').addEventListener('click',addVariable);
        box.querySelector('.cp-rename-variable').addEventListener('click',renameVariable);
        box.querySelector('.cp-edit-variable-definition').addEventListener('click',editVariableDefinition);
    }
    ...
    function addVariable() {
        openNameDialog('新しい変数名', '変数' + ((state.variables || []).length + 1), function (name) {
            ...
            if (!Array.isArray(state.variables)) state.variables = [];
            var v = { id: uid('v'), name: name, value: 0, blocks: [], connections: [] };
            state.variables.push(v);
            runtime.selectedVariableId = v.id;
            ensureVariableDefinition(v);
            refreshVariableBlockTypes();
            saveState();
            renderPalette();
            renderVariableTools();
    ...
    function renameVariable() {
        ...
        var v = state.variables.find(function(x){ return x.id === runtime.selectedVariableId; }) || state.variables[state.variables.length - 1];
        runtime.selectedVariableId = v.id;
    ...
    function normalizeSort(sort) {
        ...
        sort.blocks = sort.blocks.filter(function(b){
            if (!b || !b.id || !BLOCK_TYPES[b.type]) return false;
            valid[b.id]=true; return true;
        });
        sort.connections = sort.connections.filter(function(c){return c&&valid[c.from]&&valid[c.to];});
        var def = sort.definitionBlockId ? sort.blocks.find(function(b){return b.id===sort.definitionBlockId&&b.type==='sortDefinition';}) : null;
        if (!def) def = sort.blocks.find(function(b){return b.type==='sortDefinition';});
        if (!def) { def=defaultBlock('sortDefinition',80,80); def.sortId=sort.id; sort.blocks.push(def); }
        sort.definitionBlockId=def.id; def.sortId=sort.id; def.targetType=sort.targetType; def.order=sort.order;
        if (!def.inputs) def.inputs={value:null};
        if (def.inputs.value && !valid[def.inputs.value]) def.inputs.value=null;
        sort.blocks.forEach(function(b){if(!b.inputs)return;Object.keys(b.inputs).forEach(function(k){if(b.inputs[k]&&!valid[b.inputs[k]])b.inputs[k]=null;});});
        // 旧ソート設定は残さず、新しいソート定義へ移行する。
        var legacy=sort.blocks.find(function(b){return b.type==='sortSetting';});
        if(legacy){ if(legacy.inputs&&legacy.inputs.condition&&!def.inputs.value)def.inputs.value=legacy.inputs.condition; sort.blocks=sort.blocks.filter(function(b){return b.id!==legacy.id;}); sort.connections=sort.connections.filter(function(c){return c.from!==legacy.id&&c.to!==legacy.id;}); }
        delete sort.settingBlockId;
    ...
    function getSortConditionValue(sort, item) {
        var definition = sort && sort.blocks ? getBlock(sort, sort.definitionBlockId) : null;
        var ref = definition ? getInputRef(definition, 'value') : null;
        if (!ref) return 0;
    ...
    function createDefaultSort(name) {
        var sort = { id: uid('s'), name: name || '新しいソート', mode: 'condition', items: [], targetType: 'upgrades', order: 'desc', unavailableMode: 'next', blocks: [], connections: [], definitionBlockId: null };
        var definition = defaultBlock('sortDefinition', 80, 80);
        definition.sortId = sort.id;
        definition.targetType = sort.targetType;
        definition.order = sort.order;
        sort.blocks.push(definition);
        sort.definitionBlockId = definition.id;
        return sort;
    }
    ...
    function renderSortEditor(){
        ...
        box.innerHTML='<div style="font-size:12px;margin-top:5px">対象：<b>'+escapeHtml(getSortTargetLabel(sort))+'</b></div>' +
            '<div style="display:flex;gap:4px;margin-top:5px"><button class="cp-btn cp-edit-sort-condition" style="flex:1">ソートを定義する</button></div>' +
            '<div style="font-size:11px;margin-top:4px">ソート用の計算結果は、もし〜なら・数値変数・条件（比較や検知）・計算式を組み合わせて作成できます。</div>';
    ...
    function renderEditorModeUI(){
        ...
        var definitionMode=runtime.editorMode!=='program';
        var select=root.querySelector('.cp-program-select');
        if(select) select.disabled=definitionMode;
        root.querySelectorAll('.cp-programs + .cp-btn, .cp-programs + .cp-btn + .cp-btn').forEach(function(b){ b.disabled=definitionMode; b.style.opacity=definitionMode?'0.5':'1'; });
        var auto=root.querySelector('.cp-auto-load'); if(auto){ auto.disabled=definitionMode; auto.parentNode.style.opacity=definitionMode?'0.5':'1'; }
        var run=root.querySelector('.cp-run'); var stop=root.querySelector('.cp-stop');
        if(run) { run.disabled=definitionMode; run.style.opacity=definitionMode?'0.5':'1'; }
        if(stop) { stop.disabled=definitionMode; stop.style.opacity=definitionMode?'0.5':'1'; }
        var title=root.querySelector('.cp-block-palette-title');
        if(title) title.textContent=sortMode?'ソート定義ブロック':(runtime.editorMode==='variable'?'数値変数の定義ブロック':'ブロック');
        var modeLabel=root.querySelector('.cp-editor-mode-label'); if(modeLabel) modeLabel.style.display=definitionMode?'block':'none';
        if(modeLabel) modeLabel.textContent=sortMode?'ソート編集モード':(runtime.editorMode==='variable'?'数値変数定義モード':'ブロック編集モード');
        var tools=root.querySelector('.cp-sort-tools');
        if(tools) tools.querySelectorAll('button').forEach(function(b){ b.disabled=false; });
        var exit=root.querySelector('.cp-exit-sort-edit'); if(exit) exit.style.display=definitionMode?'block':'none';
    }
    ...
    function resolveSortInspectionTarget(id) {
        if (typeof Game === 'undefined' || !id) return null;
        if (id.indexOf('building:') === 0) {
            var bid=Number(id.slice(9));
            return Game.ObjectsById && Game.ObjectsById[bid] ? Game.ObjectsById[bid] : null;
        }
        if (id === 'special:chocolateEgg') {
            return Object.keys(Game.UpgradesById || {}).map(function(k){return Game.UpgradesById[k];}).find(function(u){return u && u.name === 'Chocolate egg';}) || null;
        }
        if (id === 'upgrade:oneMind') return Object.keys(Game.UpgradesById || {}).map(function(k){return Game.UpgradesById[k];}).find(function(u){return u && u.name === 'One mind';}) || null;
        if (id === 'upgrade:communalBrainsweep') return Object.keys(Game.UpgradesById || {}).map(function(k){return Game.UpgradesById[k];}).find(function(u){return u && u.name === 'Communal brainsweep';}) || null;
        if (id === 'upgrade:elderPact') return Object.keys(Game.UpgradesById || {}).map(function(k){return Game.UpgradesById[k];}).find(function(u){return u && u.name === 'Elder Pact';}) || null;
        var synergy = getSynergyTarget(id);
        if (synergy) return Object.keys(Game.UpgradesById || {}).map(function(k){return Game.UpgradesById[k];}).find(function(u){return u && u.name === synergy.name;}) || null;
        return null;
    }
    ...
    function getValueBlockValue(type, block) {
        ...
        if (type === 'sortTargetAcquired') {
            var evalDocAcq = runtime.sortEvaluationSort || getCurrentProgram();
            var ctxAcq = block && block.targetRef ? evaluateExpressionBlock(evalDocAcq, block.targetRef) : null;
            if (!ctxAcq || typeof ctxAcq !== 'object') ctxAcq = {type:(block&&block.target)||'', item:resolveSortInspectionTarget(block&&block.target)};
            var acquiredItem = ctxAcq.item || resolveSortInspectionTarget(ctxAcq.type);
            if (!acquiredItem) return false;
            if (ctxAcq.type && ctxAcq.type.indexOf('building:') === 0) return Number(acquiredItem.amount || 0) > 0;
            return !!acquiredItem.bought;
        }
        if (type === 'sortBuildingBaseCps' || type === 'sortBuildingCount' || type === 'sortBuildingPrice' || type === 'sortUpgradePrice' || type === 'sortBuildingUpgradeMultiplier') {
            var evalDoc = runtime.sortEvaluationSort || getCurrentProgram();
            var ctx = block && block.targetRef ? evaluateExpressionBlock(evalDoc, block.targetRef) : null;
            if (!ctx || typeof ctx !== 'object') ctx = {type:(block&&block.target)||'', item:resolveSortInspectionTarget(block&&block.target)};
            var item=ctx.item || resolveSortInspectionTarget(ctx.type);
            if(type==='sortBuildingBaseCps') return item && item.baseCps != null ? Number(item.baseCps)||0 : 0;
            if(type==='sortBuildingCount') return item && item.amount != null ? Number(item.amount)||0 : 0;
            if(type==='sortBuildingPrice') return item && item.price != null ? Number(item.price)||0 : 0;
            if(type==='sortUpgradePrice') return item && item.getPrice ? Number(item.getPrice())||0 : (item&&item.basePrice!=null?Number(item.basePrice)||0:(item&&item.price!=null?Number(item.price)||0:0));
            if(type==='sortBuildingUpgradeMultiplier') return item && item.totalCpsMult ? Number(item.totalCpsMult)||0 : 1;
        }
        ...
        if (type.indexOf('variable:') === 0) {
            ...
            return evaluateNumericVariable(v);
        }
        return 0;
    }


    function evaluateNumericVariable(v, seen) {
        if(!v)return 0;
        seen=seen||{}; if(seen[v.id])return 0; seen[v.id]=true;
        var root=ensureVariableDefinition(v);
        var ref=root&&getInputRef(root,'value');
        if(ref){ var old=runtime.evaluatingVariables||{}; if(old[v.id]) return Number(v.value)||0; runtime.evaluatingVariables=old; old[v.id]=true; var out=Number(evaluateExpressionBlock(v,ref,seen))||0; delete old[v.id]; return out; }
    ...
    function evaluateExpressionBlock(p, id, seen) {
        ...
        if (!def) return 0;
        if (b.type === 'branch') {
            var cond = getInputRef(b,'condition') ? !!evaluateExpressionBlock(p,getInputRef(b,'condition')) : false;
            var key = cond ? 'trueValue' : 'falseValue';
            return getInputRef(b,key) ? evaluateExpressionBlock(p,getInputRef(b,key)) : readNumber(b[key],p,b.id,key);
        }
        if (b.type === 'sortTarget') return {type:b.target || '', item:resolveSortInspectionTarget(b.target)};
    ...
    function bindEvents(root) {
        ...
        wrap.addEventListener('touchmove', function (ev) {
            ...
            if (ev.touches.length === 1 && g.touches.length === 1) {
                var dx1 = ev.touches[0].clientX - g.lastX;
                var dy1 = ev.touches[0].clientY - g.lastY;
                var bounds1 = getViewBounds();
                var nextY1 = runtime.view.y + dy1;
                var excessY1 = 0;
                if (bounds1) {
                    if (nextY1 > bounds1.maxY) {
                        excessY1 = nextY1 - bounds1.maxY;
                        nextY1 = bounds1.maxY;
                    } else if (nextY1 < bounds1.minY) {
                        excessY1 = nextY1 - bounds1.minY;
                        nextY1 = bounds1.minY;
                    }
                }
                runtime.view.x += dx1;
                runtime.view.y = nextY1;
                applyView();
                if (excessY1) handoffToCookieClicker(excessY1);
                g.lastX = ev.touches[0].clientX;
                g.lastY = ev.touches[0].clientY;
                return;
            }
        ...
        wrap.addEventListener('pointermove', function (ev) {
            ...
            if (runtime.pan && runtime.pan.pointerId === ev.pointerId) {
                var dxPan = ev.clientX - runtime.pan.lastX;
                var dyPan = ev.clientY - runtime.pan.lastY;
                var boundsPan = getViewBounds();
                var nextYPan = runtime.view.y + dyPan;
                var excessYPan = 0;
                if (boundsPan) {
                    if (nextYPan > boundsPan.maxY) {
                        excessYPan = nextYPan - boundsPan.maxY;
                        nextYPan = boundsPan.maxY;
                    } else if (nextYPan < boundsPan.minY) {
                        excessYPan = nextYPan - boundsPan.minY;
                        nextYPan = boundsPan.minY;
                    }
                }
                runtime.view.x += dxPan;
                runtime.view.y = nextYPan;
                runtime.pan.lastX = ev.clientX;
                runtime.pan.lastY = ev.clientY;
                applyView();
                if (excessYPan) handoffToCookieClicker(excessYPan);
            }
        });
