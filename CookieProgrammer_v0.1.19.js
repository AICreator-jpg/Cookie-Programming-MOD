...
(function () {
    ...
    var RESET_MARKER_KEY = 'CookieProgrammerMod.v01.reset_0_1_19';
    var ROOT_ID = 'cp-root';
...
<div class="cp-window">
  ...
  <div class="cp-body">
    <aside class="cp-sidebar">
      ...
      <div class="cp-sort-tools" style="margin-top:8px"><b>ソート</b><div style="display:flex;gap:4px;margin-top:4px"><button class="cp-btn cp-add-sort" style="flex:1">追加</button><button class="cp-btn cp-rename-sort" style="flex:1">名前変更</button><button class="cp-btn cp-delete-sort" style="flex:1">削除</button></div><div class="cp-sort-list"></div><div class="cp-sort-editor"></div></div>
      ...
      <div class="cp-help-source">配置：ブロックをタップ/クリックすると追加。追加後はドラッグで移動できます。<br><br>接続：先に実行するブロックをクリック → 後に実行するブロックをクリック。複数接続できます。<br><br>もし〜なら：条件に合えば1本目、合わなければ2本目の接続へ進みます。<br><br>演算：数値欄には直接入力でき、演算・情報・変数・検知ブロックも入れ子にできます。<br><br>検知：かつ・または・ではないを使って条件を組み合わせられます。<br><br>削除：ブロックを選択してブロック削除。PCではDeleteキーでも削除できます。<br><br>ページ：各カテゴリは8ブロックごとにページが増えます。</div>
    </aside>
  ...
  <div class="cp-modal">
    ...
    function blockInnerHtml(b) {
        ...
        if (b.type === 'buyUpgrade') {
            var sortOptions = (state.sorts || []).map(function(s){ return {id:s.id,label:s.name}; });
    ...
    function normalizeSort(sort) {
        if (!sort) return;
        if (!sort.mode) sort.mode = 'manual';
        if (!Array.isArray(sort.items)) sort.items = [];
        if (!sort.condition) sort.condition = { type: 'cookiesPs', value: '0' };
    ...
    function getUpgradeList() {
        if (typeof Game === 'undefined' || !Game.UpgradesById) return [];
        return Object.keys(Game.UpgradesById).map(function(k){ return Game.UpgradesById[k]; }).filter(function(u){ return !!u; });
    }

    function getUpgradeValue(upgrade) {
        var sort = runtime.sortEvaluationSort;
        if (!sort || !sort.condition) return 0;
        if (sort.condition.kind === 'info') return getValueBlockValue(sort.condition.type, sort.condition.block || {});
        return Number(upgrade && upgrade.basePrice) || 0;
    }

    function getSortedUpgrades(sort) {
        normalizeSort(sort);
        var all = getUpgradeList().filter(function(u){ return !u.bought; });
        var excluded = {};
        sort.items.forEach(function(x){ if (x.excluded) excluded[x.id] = true; });
        all = all.filter(function(u){ return !excluded[String(u.id)]; });
        if (sort.mode === 'manual') {
            var order = {};
            sort.items.forEach(function(x,i){ order[x.id] = i; });
        ...
        all.sort(function(a,b){
            var av = Number(a.basePrice)||0, bv = Number(b.basePrice)||0;
            if (sort.order === 'desc') return bv-av;
            if (sort.order === 'near0') return Math.abs(av)-Math.abs(bv);
            if (sort.order === 'far0') return Math.abs(bv)-Math.abs(av);
            return av-bv;
        });
    ...
    function executeUpgradeSort(sortId, unavailableMode) {
        ...
        var list = getSortedUpgrades(sort);
    ...
    function createDefaultSort(name) {
        var sort = { id: uid('s'), name: name || '新しいソート', mode: 'manual', items: [], condition: { type:'cookiesPs', value:'0' }, order:'asc', unavailableMode:'next' };
        getUpgradeList().forEach(function(u){ sort.items.push({id:String(u.id), name:u.name || ('アップグレード'+u.id), excluded:false}); });
        return sort;
    }
    ...
    function renderSortManager() {
        ...
        (state.sorts || []).forEach(function(sort){
            ...
            row.innerHTML='<button class="cp-sort-select" style="flex:1;text-align:left">'+escapeHtml(sort.name)+'</button>';
            row.querySelector('.cp-sort-select').addEventListener('click',function(){ runtime.selectedSortId=sort.id; renderSortManager(); renderSortEditor(); });
            list.appendChild(row);
        });
        var selected=getSort(runtime.selectedSortId);
        if (selected) selected.__ui=true;
    }

    function renderSortEditor(){
        var root=document.getElementById(ROOT_ID); if(!root)return;
        var box=root.querySelector('.cp-sort-editor'); if(!box)return;
        var sort=getSort(runtime.selectedSortId); if(!sort){box.innerHTML='ソートを選択してください';return;}
        normalizeSort(sort);
        var rows=sort.items.slice(0,30).map(function(x,i){ return '<div class="cp-sort-item"><span>'+escapeHtml(x.name)+'</span><button data-sort-index="'+i+'" class="cp-sort-item-up">↑</button><button data-sort-index="'+i+'" class="cp-sort-item-down">↓</button><button data-sort-index="'+i+'" class="cp-sort-item-exclude">'+(x.excluded?'除外中':'除外')+'</button></div>'; }).join('');
        box.innerHTML='<label>方式 <select class="cp-sort-mode"><option value="manual">手動</option><option value="condition">条件</option></select></label><div class="cp-sort-condition" style="margin-top:5px">条件: アップグレード価格を基準に並べます</div><label style="display:block;margin-top:5px">順序 <select class="cp-sort-order"><option value="asc">昇順</option><option value="desc">降順</option><option value="near0">0に近い順</option><option value="far0">0から遠い順</option></select></label><div class="cp-sort-items">'+rows+'</div>';
        box.querySelector('.cp-sort-mode').value=sort.mode;
        box.querySelector('.cp-sort-order').value=sort.order;
        box.querySelector('.cp-sort-mode').addEventListener('change',function(){sort.mode=this.value;saveState();renderSortEditor();});
        box.querySelector('.cp-sort-order').addEventListener('change',function(){sort.order=this.value;saveState();});
        box.querySelectorAll('.cp-sort-item-up').forEach(function(b){b.addEventListener('click',function(){reorderSortItem(sort,Number(this.dataset.sortIndex),-1);});});
        box.querySelectorAll('.cp-sort-item-down').forEach(function(b){b.addEventListener('click',function(){reorderSortItem(sort,Number(this.dataset.sortIndex),1);});});
        box.querySelectorAll('.cp-sort-item-exclude').forEach(function(b){b.addEventListener('click',function(){var i=Number(this.dataset.sortIndex);sort.items[i].excluded=!sort.items[i].excluded;saveState();renderSortEditor();});});
    }
    ...
    function bindEvents(root) {
        ...
        root.querySelector('.cp-add-sort').addEventListener('click', function(){ openNameDialog('新しいソート名','新しいソート',function(name){var sort=createDefaultSort(name);state.sorts.push(sort);runtime.selectedSortId=sort.id;saveState();renderSortManager();renderSortEditor();renderPalette();renderCanvas();}); });
