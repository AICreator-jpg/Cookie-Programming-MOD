...
<div class="cp-window">
  ...
  <div class="cp-body">
    <aside class="cp-sidebar">
      ...
      <div class="cp-help">
        ...
        <b>演算：</b>入力欄をクリックしてから演算・情報ブロックをクリックすると入れられます。演算ブロックは入れ子にできます。<br><br>        <b>検知：</b>「かつ」「または」「ではない」は条件を組み合わせて入れ子にできます。<br><br>
  ...
  <div class="cp-modal">
    ...
    function isExpressionType(type) {
        return ['value','operator','comparison','logic'].indexOf(BLOCK_TYPES[type] ? BLOCK_TYPES[type].kind : '') >= 0;
    }

    function isConditionType(type) {
        return !!(BLOCK_TYPES[type] && ['comparison','logic'].indexOf(BLOCK_TYPES[type].kind) >= 0);
    }
    ...
    function inputSlotHtml(p, b, key, seen) {
        ...
        var fallback = key === 'a' ? b.a : (key === 'b' ? b.b : (key === 'value' ? b.value : '0'));
    ...
    function executePurchaseTarget(target) {
        if (typeof Game === 'undefined' || !target || target.indexOf('building:') !== 0) return false;
