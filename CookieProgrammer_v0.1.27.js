...
(function () {
    ...
    function injectStyle() {
        ...
        style.id = STYLE_ID;
        style.textContent = `
#${ROOT_ID}{position:relative;width:100%;height:100%;max-width:100%;max-height:100%;min-width:260px;min-height:260px;z-index:1;display:flex;font-family:Arial,"Trebuchet MS",sans-serif;color:#333;touch-action:auto;box-sizing:border-box;flex:0 0 auto}
#${ROOT_ID} .cp-dim{display:none}
#${ROOT_ID} .cp-window{position:relative;width:100%;height:100%;min-width:0;min-height:0;background:#eee;border:0;border-radius:0;box-shadow:none;overflow:hidden;display:flex;flex-direction:column}
...
#${ROOT_ID} .cp-palette-list-wrap{flex:1;min-height:80px;overflow-y:auto;overflow-x:hidden;background:rgba(0,0,0,.55);padding:4px 6px;border-radius:5px;overscroll-behavior-y:auto;-webkit-overflow-scrolling:touch;touch-action:pan-y}
...
`;
    ...
    function fitRootSquare() {
        ...
        if (!pw || !ph) return;
        var size = Math.floor(Math.min(pw, ph));
        if (size < 260) size = Math.min(pw, ph);
        if (size <= 0) return;
        root.style.width = size + 'px';
        root.style.height = size + 'px';
        root.style.flex = '0 0 ' + size + 'px';
...
<div class="cp-window">
  ...
  <div class="cp-modal">
    ...
    function bindEvents(root) {
        ...
        // Touch操作はPointer Events側に一本化する。
        // touchイベントとpointerイベントを同時に処理すると、Androidでは移動量が二重に加算されて
        // スワイプが異常に速くなるため、ここではtouchstart/touchmove等を登録しない。
