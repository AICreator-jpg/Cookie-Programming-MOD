...
(function () {
    ...
    var RESET_MARKER_KEY = 'CookieProgrammerMod.v01.reset_0_1_15';
    var ROOT_ID = 'cp-root';
...
<div class="cp-window">
  ...
  <div class="cp-modal">
    ...
    function inputSlotHtml(p, b, key) {
        ...
        return '<input class="cp-input-slot cp-input-field" type="text" inputmode="decimal" data-input-owner="' + escapeHtml(b.id) + '" data-input-key="' + key + '" value="' + escapeHtml(fallback == null ? '0' : fallback) + '">';
    }
    ...
    function bindBlockFields(el, block) {
        ...
        el.querySelectorAll('.cp-input-slot').forEach(function (slot) {
            slot.addEventListener('pointerdown', function (ev) {
                ev.stopPropagation();
                if (this.classList.contains('cp-input-field')) {
            ...
            slot.addEventListener('input', function () {
                var owner = getBlock(getCurrentProgram(), this.dataset.inputOwner);
                if (!owner) return;
                owner[this.dataset.inputKey] = this.value;
                saveState();
            });
