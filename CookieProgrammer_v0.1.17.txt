...
(function () {
    ...
    var RESET_SAVED_DATA_ONCE = true;
    var RESET_MARKER_KEY = 'CookieProgrammerMod.v01.reset_0_1_17';
    var ROOT_ID = 'cp-root';
...
<div class="cp-window">
  ...
  <div class="cp-modal">
    ...
    function isConditionType(type) {
        return !!(BLOCK_TYPES[type] && BLOCK_TYPES[type].kind === 'comparison');
    }
    ...
    function expressionLabel(p, id, seen) {
        ...
        if (def.kind === 'value') {
            ...
            if (b.type === 'prestige') valueLabel = 'プレステージ（' + ({level:'レベル',chips:'ヘブンリーチップ',gain:'今回獲得',resets:'転生回数'}[b.info] || 'レベル') + '）';
            if (b.type === 'goldenCookies') valueLabel = 'ゴールデンクッキー（' + ({clicked:'クリック数',spawned:'出現回数',active:'出現中'}[b.info] || 'クリック数') + '）';
            if (b.type === 'inventory') valueLabel = getPurchaseTargetLabel(b.target) + 'の所持数';
            return '<span class=\"cp-nested-expression cp-expression-child\" data-expression-id=\"' + escapeHtml(b.id) + '\">' + escapeHtml(valueLabel) + '</span>';
        }
        ...
        if (def.kind === 'operator' || def.kind === 'comparison') {
            ...
            return '<span class=\"cp-nested-expression cp-expression-child\" data-expression-id=\"' + escapeHtml(b.id) + '\">' + a + ' ' + symbol + ' ' + c + '</span>';
        }
    ...
    function inputSlotHtml(p, b, key) {
        ...
        if (ref) {
            return '<span class="cp-input-slot cp-input-expression" data-input-owner="' + escapeHtml(b.id) + '" data-input-key="' + key + '">' + expressionLabel(p, ref) + '</span>';
        }
        var fallback = key === 'left' ? b.left : (key === 'right' ? b.right : (key === 'a' ? b.a : b.b));
        return '<span class="cp-input-slot cp-input-empty" data-input-owner="' + escapeHtml(b.id) + '" data-input-key="' + key + '">' + escapeHtml(fallback == null ? '0' : fallback) + '</span>';
    }
    ...
    function getPurchaseTargets() {
        ...
        if (typeof Game !== 'undefined' && Game.UpgradesById) {
            Object.keys(Game.UpgradesById).forEach(function (key) {
                var up = Game.UpgradesById[key];
                if (up) targets.push({ id: 'upgrade:' + up.id, label: 'アップグレード: ' + up.name });
            });
        }
        return targets;
    }
    ...
    function bindBlockFields(el, block) {
        ...
        el.querySelectorAll('.cp-input-slot').forEach(function (slot) {
            ...
            slot.addEventListener('pointerup', function (ev) {
                ...
                // 入力スロットをタップしたら「ここに次の式ブロックを入れる」
                // という対象にする。キーボード入力はこの操作では行わない。
                runtime.expressionTarget = {
                    parentId: this.dataset.inputOwner,
                    key: this.dataset.inputKey
                };
                runtime.connectionFrom = null;
    ...
    function evaluateCondition(b, p) {
        ...
        if (!conditionBlock || !BLOCK_TYPES[conditionBlock.type] || BLOCK_TYPES[conditionBlock.type].kind !== 'comparison') return false;
