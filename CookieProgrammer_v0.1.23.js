...
(function () {
    ...
    var runtime = {
        ...
        selectedConditionVariableId: null,
        editorMode: 'program'
    };
    ...
    var BLOCK_TYPES = {
        ...
        conditionVariableDefinition: { label: '条件変数を定義する', kind: 'conditionVariableDefinition', color: '#c58bd8', category: 'control' },
    ...
    function refreshConditionVariableBlockTypes() {
        Object.keys(BLOCK_TYPES).forEach(function (type) {
            if (type.indexOf('conditionVariable:') === 0) delete BLOCK_TYPES[type];
        });
        (state.conditionVariables || []).forEach(function (v) {
            if (!v || !v.id) return;
            BLOCK_TYPES['conditionVariable:' + v.id] = {
                label: v.name || '条件変数',
                kind: 'logic',
                color: '#c58bd8',
                category: 'variables',
                conditionVariableId: v.id
            };
        });
    }

    function getConditionVariable(id) {
        return (state.conditionVariables || []).find(function (v) { return v.id === id; }) || null;
    }
    ...
    function defaultBlock(type, x, y) {
        ...
        if (type === 'conditionVariableDefinition') {
            block.conditionVariableId = state.conditionVariables[0] ? state.conditionVariables[0].id : '';
            block.inputs = { condition: null };
        }
        ...
        if (type === 'sortDefinition') { block.sortId = state.sorts[0] ? state.sorts[0].id : ''; block.targetType = 'upgrades'; block.order = 'desc'; block.inputs = { value: null }; }
    ...
    function sanitizeState() {
        ...
        if (!Array.isArray(state.conditionVariables)) state.conditionVariables = [];
        state.conditionVariables = state.conditionVariables.filter(function (v) { return v && v.id && v.name; });
        refreshVariableBlockTypes();
        refreshConditionVariableBlockTypes();
        state.variables.forEach(function(v){ ensureVariableDefinition(v); });
        state.conditionVariables.forEach(function(v){ ensureConditionVariableDefinition(v); });
    ...
    function loadState() {
        try {
            ...
            state.conditionVariables = Array.isArray(parsed.conditionVariables) ? parsed.conditionVariables : [];
    ...
    function getEditorDocument() {
        ...
        if (runtime.editorMode === 'conditionVariable') return getConditionVariable(runtime.selectedConditionVariableId);
...
<div class="cp-window">
  ...
  <div class="cp-modal">
    ...
    function expressionInputKeys(b) {
        if (!b) return [];
        if (b.type === 'if' || b.type === 'conditionVariableDefinition') return ['condition'];
        ...
        if (['sortBuildingBaseCps','sortBuildingCount','sortBuildingPrice','sortUpgradePrice','sortBuildingUpgradeMultiplier'].indexOf(b.type)>=0) return ['targetRef'];
        return [];
    }
    ...
    function expressionLabel(p, id, seen) {
        ...
        var def = BLOCK_TYPES[b.type];
        if (!def) return '';
        if (b.type.indexOf('conditionVariable:') === 0) {
            return '<span class="cp-nested-expression cp-expression-child" data-expression-id="' + escapeHtml(b.id) + '" style="background:' + escapeHtml(def.color) + '">' + escapeHtml(def.label) + '</span>';
        }
        if (b.type === 'sortTarget') {
            ...
            return '<span class="cp-nested-expression cp-expression-child" data-expression-id="' + escapeHtml(b.id) + '" style="background:' + escapeHtml(def.color) + '">ソート対象：' + escapeHtml(targetLabels[b.target] || '対象') + '</span>';
        }
        if (b.type === 'branch') {
    ...
    function blockInnerHtml(b) {
        ...
        if (b.type === 'conditionVariableDefinition') {
            var cv = getConditionVariable(b.conditionVariableId);
            return head + '<div style="text-align:center"><b>条件変数「' + escapeHtml(cv ? cv.name : '条件変数') + '」を定義する</b></div><div class="cp-expression-row">＝ ' + inputSlotHtml(p,b,'condition') + '</div>';
        }
        ...
        if (['sortBuildingBaseCps','sortBuildingCount','sortBuildingPrice','sortUpgradePrice','sortBuildingUpgradeMultiplier'].indexOf(b.type)>=0) {
    ...
    function getPaletteTypes() {
        return Object.keys(BLOCK_TYPES).filter(function (type) {
            ...
            if (runtime.editorMode === 'conditionVariable') {
                if (runtime.paletteCategory === 'control') return false;
                if (['operator','info','variables','detection'].indexOf(runtime.paletteCategory) < 0) return false;
                return def.category === runtime.paletteCategory && ['value','operator','comparison','logic','detection'].indexOf(def.kind) >= 0;
            }
    ...
    function addBlock(type, x, y) {
        ...
        if (runtime.editorMode === 'sort') {
            ...
            if (type !== 'sortDefinition' && ['value','operator','comparison','logic','detection'].indexOf(def.kind) < 0) { setStatus('ソートには値・演算・変数・条件ブロックを使ってください'); return; }
        ...
        if (runtime.editorMode === 'variable' && type !== 'variableDefinition') {
            var vd = BLOCK_TYPES[type];
            if (!vd || ['value','operator','comparison','logic','detection'].indexOf(vd.kind) < 0) return;
        }
        if (runtime.editorMode === 'conditionVariable' && type !== 'conditionVariableDefinition') {
            var cd = BLOCK_TYPES[type];
            if (!cd || ['value','operator','comparison','logic','detection'].indexOf(cd.kind) < 0) return;
        }
        ...
        if (runtime.editorMode === 'sort' && type === 'sortDefinition') {
            b.sortId = p.id; b.targetType = p.targetType || 'upgrades'; b.order = p.order || 'desc'; p.definitionBlockId = b.id;
        }
        p.blocks.push(b);
        ...
        if (runtime.expressionTarget && isExpressionType(type)) {
            ...
            var allowed = true;
            if (targetParent && ['if','conditionVariableDefinition'].indexOf(targetParent.type) >= 0 && runtime.expressionTarget.key === 'condition') {
                allowed = isConditionType(type);
            }
            if (targetParent && targetParent.type === 'branch' && runtime.expressionTarget.key === 'condition') {
                allowed = isConditionType(type);
            }
    ...
    function ensureConditionVariableDefinition(v) {
        if (!v) return null;
        if (!Array.isArray(v.blocks)) v.blocks = [];
        if (!Array.isArray(v.connections)) v.connections = [];
        var root = v.blocks.find(function(b){ return b.type === 'conditionVariableDefinition'; });
        if (!root) { root = defaultBlock('conditionVariableDefinition',80,80); root.conditionVariableId=v.id; v.blocks.push(root); }
        v.definitionRootId = root.id;
        return root;
    }

    function addConditionVariable() {
        openNameDialog('新しい条件変数名','条件' + ((state.conditionVariables||[]).length+1),function(name){
            if ((state.conditionVariables||[]).some(function(v){return v.name===name;})) { setStatus('同じ名前の条件変数があります'); return; }
            if (!Array.isArray(state.conditionVariables)) state.conditionVariables=[];
            var v={id:uid('cv'),name:name,blocks:[],connections:[]};
            state.conditionVariables.push(v); ensureConditionVariableDefinition(v); refreshConditionVariableBlockTypes(); saveState(); renderPalette(); renderCanvas(); renderVariableTools(); setStatus('条件変数を追加しました');
        });
    }

    function renameConditionVariable() {
        var list=state.conditionVariables||[]; if(!list.length){setStatus('条件変数がありません');return;}
        var v=list.find(function(x){return x.id===runtime.selectedConditionVariableId;}) || list[list.length-1];
        runtime.selectedConditionVariableId=v.id;
        openNameDialog('条件変数名を変更',v.name,function(name){
            if(list.some(function(x){return x.id!==v.id&&x.name===name;})){setStatus('同じ名前の条件変数があります');return;}
            v.name=name; refreshConditionVariableBlockTypes(); saveState(); renderPalette(); renderCanvas(); renderVariableTools(); setStatus('条件変数名を変更しました');
        });
    }
    ...
    function editConditionVariableDefinition() {
        var list=state.conditionVariables||[]; if(!list.length){setStatus('条件変数がありません');return;}
        var v=list[state.conditionVariables.findIndex(function(x){return x.id===runtime.selectedConditionVariableId;})] || list[list.length-1];
        runtime.selectedConditionVariableId=v.id; ensureConditionVariableDefinition(v); runtime.editorMode='conditionVariable'; runtime.selectedBlockId=null; runtime.connectionFrom=null; runtime.expressionTarget=null; renderVariableTools(); renderEditorModeUI(); renderPalette(); renderCanvas(); setStatus('条件変数「'+v.name+'」の定義を編集しています');
    }
    ...
    function renderVariableTools() {
        ...
        var nc=(state.conditionVariables||[]).map(function(v){return {id:v.id,label:v.name};});
        if(!runtime.selectedVariableId && nv.length) runtime.selectedVariableId=nv[0].id;
        if(!runtime.selectedConditionVariableId && nc.length) runtime.selectedConditionVariableId=nc[0].id;
        box.innerHTML='<div style="width:100%"><b>数値変数</b><div style="display:flex;gap:3px;margin-top:3px"><select class="cp-select cp-variable-select" style="flex:1">'+optionsHtml(nv,runtime.selectedVariableId|| (nv[0]&&nv[0].id))+'</select><button class="cp-btn cp-add-variable">追加</button><button class="cp-btn cp-rename-variable">名前変更</button><button class="cp-btn cp-edit-variable-definition">定義</button></div></div>'
            +'<div style="width:100%;margin-top:6px"><b>条件変数</b><div style="display:flex;gap:3px;margin-top:3px"><select class="cp-select cp-condition-variable-select" style="flex:1">'+optionsHtml(nc,runtime.selectedConditionVariableId||(nc[0]&&nc[0].id))+'</select><button class="cp-btn cp-add-condition-variable">追加</button><button class="cp-btn cp-rename-condition-variable">名前変更</button><button class="cp-btn cp-edit-condition-variable-definition">定義</button></div></div>';
        ...
        var cs=box.querySelector('.cp-condition-variable-select'); if(cs)cs.addEventListener('change',function(){runtime.selectedConditionVariableId=this.value;});
        ...
        box.querySelector('.cp-add-condition-variable').addEventListener('click',addConditionVariable);
        box.querySelector('.cp-rename-condition-variable').addEventListener('click',renameConditionVariable);
        box.querySelector('.cp-edit-condition-variable-definition').addEventListener('click',editConditionVariableDefinition);
    }
    ...
    function renderSortEditor(){
        ...
        box.innerHTML='<div style="font-size:12px;margin-top:5px">対象：<b>'+escapeHtml(getSortTargetLabel(sort))+'</b></div>' +
            ...
            '<div style="font-size:11px;margin-top:4px">ソート用の計算結果は、条件変数・もし〜なら・数値変数・計算式を組み合わせて作成できます。</div>';
    ...
    function renderEditorModeUI(){
        ...
        if(title) title.textContent=sortMode?'ソート定義ブロック':(runtime.editorMode==='variable'?'数値変数の定義ブロック':(runtime.editorMode==='conditionVariable'?'条件変数の定義ブロック':'ブロック'));
        var modeLabel=root.querySelector('.cp-editor-mode-label'); if(modeLabel) modeLabel.style.display=definitionMode?'block':'none';
        if(modeLabel) modeLabel.textContent=sortMode?'ソート編集モード':(runtime.editorMode==='variable'?'数値変数定義モード':'条件変数定義モード');
    ...
    function evaluateConditionVariable(v, seen) {
        if (!v) return false;
        seen=seen||{}; if(seen[v.id])return false; seen[v.id]=true;
        var root=ensureConditionVariableDefinition(v);
        var ref=root&&getInputRef(root,'condition');
        if(!ref)return false;
        return !!evaluateExpressionBlock(v,ref,seen);
    }
    ...
    function evaluateExpressionBlock(p, id, seen) {
        ...
        var def = BLOCK_TYPES[b.type];
        if (!def) return 0;
        if (b.type.indexOf('conditionVariable:') === 0) return evaluateConditionVariable(getConditionVariable(b.type.slice(17)));
        if (b.type === 'branch') {
