...
(function () {
    ...
    function injectStyle() {
        ...
        style.id = STYLE_ID;
        style.textContent = `
#${ROOT_ID}{position:fixed;inset:0;z-index:2147483000;display:flex;align-items:center;justify-content:center;font-family:Arial,"Trebuchet MS",sans-serif;color:#333;touch-action:none}
#${ROOT_ID} .cp-dim{position:absolute;inset:0;background:rgba(0,0,0,.45)}
#${ROOT_ID} .cp-window{position:relative;width:min(1100px,96vw);height:min(760px,94vh);background:#eee;border:4px solid #555;border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,.5);overflow:hidden;display:flex;flex-direction:column}
